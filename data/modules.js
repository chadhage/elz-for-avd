...
`,
    ...
    currentState: [
      ...
      { id: "current_broker", type: "select", label: "Current VDI broker", options: ["None (greenfield)", "AVD (commercial)", "Citrix", "VMware Horizon", "Windows Server RDS", "Other"] },
      ...
      { id: "current_host_os", type: "text", label: "Current session-host OS", placeholder: "e.g., Windows 11 multi-session; Windows Server 2022" },
...
`,
    ...
    currentState: [
      ...
      { id: "waf_net_security", type: "checkbox", label: "Network security — what measures have you implemented to improve network security?", help: "GCC High first: use the Azure Government required-URL list and service tags; confirm Azure Firewall and Private Link availability in Gov. AVD second: NSG/ASG segmentation, forced tunneling, private endpoints, and web filtering.", options: ["We understand the configurations that are needed to use Azure Firewall in our deployment.", "To isolate our subnets and help control their traffic flows, we've created network security groups and application security groups to segment our Azure Virtual Desktop traffic.", "To avoid frequently updating network security rules, we use service tags instead of specific IP addresses for Azure services.", "We're familiar with the required URLs for Azure Virtual Desktop.", "We use forced tunneling to route traffic to a firewall or network virtual appliance (NVA), and we use a route table to allow Azure Virtual Desktop traffic to bypass the forced tunneling rule.", "We use private endpoints to help protect platform as a service (PaaS) solutions like Azure Files and Azure Key Vault, and we're aware of the cost of using private endpoints.", "To avoid using public IP addresses, we've adjusted the configuration options for Azure Private Link by disabling public endpoints for Azure Virtual Desktop control plane components and using private endpoints.", "We use Active Directory Domain Services (AD DS), and we've implemented strict firewall policies that are based on the traffic that's required across our domain.", "To help protect our end users' access to the internet from Azure Virtual Desktop session hosts, we use Azure Firewall or network virtual appliance (NVA) web filtering.", "None of the above"] },
      ...
      { id: "waf_iam_network_security", type: "checkbox", label: "Network security implementation — how do you implement network security in AVD?", help: "GCC High first: hub-spoke isolation within the Gov boundary, block direct RDP, and use Azure Bastion for admin access. AVD second: NSGs, host-pool isolation, Private Link/ExpressRoute, and Azure Firewall.", options: ["We use a hub-spoke architecture to keep workload-specific resources in their own virtual network that's separate from hub shared services like management and Domain Name System (DNS) services.", "We use network security groups to filter network traffic to and from our Azure Virtual Desktop workload.", "We isolate our host pools by placing each host pool in a separate virtual network. We use network security groups with the URLs that Azure Virtual Desktop requires for each subnet.", "We rigorously enforce network and application security.", "We disable or block the Remote Desktop Protocol (RDP) port to avoid direct RDP access to session hosts. If we need direct RDP access for administrative purposes or troubleshooting, we use Azure Bastion to connect to session hosts.", "We use Azure Private Link with Azure Virtual Desktop to keep traffic within the Microsoft network and avoid exposing our service to the public internet, or we use a virtual private network (VPN) or Azure ExpressRoute so that users with a Remote Desktop client can connect to our virtual network.", "We use Azure Firewall to help protect Azure Virtual Desktop.", "None of the above"] },
...
`,
    ...
    currentState: [
      ...
      /* --- Azure WAF for AVD assessment (GCC High-framed, from WAF AVD workload CSV) --- */
      { id: "waf_shared_responsibility_ops", type: "checkbox", label: "Shared responsibility (operations) — do you know which components Microsoft manages and which you manage?", help: "GCC High first: understand customer responsibilities under the Azure Government shared-responsibility model. AVD second: network topology, session hosts, and workspace.", options: ["We're aware of our responsibilities under the shared-responsibility model.", "We ensure that our organization actively manages components that fall under the customer's responsibility, such as our network topology, our session hosts, and our workspace.", "None of the above"] },
      ...
      { id: "waf_shared_responsibility_bc", type: "checkbox", label: "Shared responsibility (business continuity) — do you know which areas Microsoft manages and which you manage?", help: "GCC High first: understand customer-managed continuity areas under the Gov shared-responsibility model. AVD second: the VMs AVD uses, user profile data, and environment-specific settings.", options: ["We're aware of our responsibilities under the shared-responsibility model.", "We ensure that our organization actively manages areas that fall under the customer's responsibility, such as the virtual machines (VMs) that Azure Virtual Desktop uses, the user profile data, and settings that are unique to our environment.", "None of the above"] }
    ],
    toBeState: [
      ...
      { id: "raci_platform", type: "text", label: "Platform owner", placeholder: "Team / role" },
      { id: "raci_workload", type: "text", label: "Workload owner", placeholder: "Team / role" },
      { id: "raci_security", type: "text", label: "Security owner", placeholder: "Team / role" },
      { id: "raci_compliance", type: "text", label: "Compliance owner", placeholder: "Team / role" },
      { id: "raci_operations", type: "text", label: "Operations owner", placeholder: "Team / role" },
